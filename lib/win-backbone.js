...
function winBB(homeDirectory)
{
	...
	self.log = function()
	{
		...
		if(typeof arguments[0] == "number")
		{
			...
			logCategory = self.log.verbose;

		if(logCategory <= self.log.logLevel)
	...
	self.loadModules = function(inputNameOrObject, configurations)
	{
		...
		for(var key in moduleObjects)
		{
			...
			{
				self.log('Module does not implement winFunction properly-- must be non-empty string unlike: ' +  wFun);
				throw new Error("Improper win function");
			}
			if(callerEvents[wFun])
			{
				self.log('Duplicate win functionality in modules -- can only have 1 ' +  wFun);
	...
	self.emit = function()
	{
		...
		self.log("[" + caller + "]", "calling", "[" + parsed.winFunction + "]->" + eventName);
